...
class ServerDownloader:
    ...
    def get_versions(self, server_type: str) -> list[str]:
        ...
        if server_type == "paper":
            data = expect_object(
                self.http.json("https://api.papermc.io/v2/projects/paper"),
                "Paper",
            )
            version_items = expect_list(data.get("versions"), "versiones de Paper")
            versions = [item for item in version_items if isinstance(item, str)]
    ...
    def download(self, server_type: str, version: str, root: Path) -> Path:
        ...
        if server_type == "paper":
            build_data = expect_object(
                self.http.json(
                    f"https://api.papermc.io/v2/projects/paper/versions/{version}"
                ),
                "builds de Paper",
            )
            build_items = expect_list(build_data.get("builds"), "builds de Paper")
            builds = [item for item in build_items if isinstance(item, int)]
            if not builds:
